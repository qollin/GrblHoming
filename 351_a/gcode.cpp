...
void GCode::grblSetHome()
{
    ...
    if (numaxis == MAX_AXIS_COUNT)
		gotoXYZA("G92 x0 y0 z0 c0");
	else
		gotoXYZA("G92 x0 y0 z0");
}
...
void GCode::goToHome()
{
    ...
    gotoXYZA(QString("G0 z").append(zpos));

    if (numaxis == MAX_AXIS_COUNT)
		gotoXYZA("G1 x0 y0 z0 c0");
	else
		gotoXYZA("G1 x0 y0 z0");
	...
	if (good) {  /// naxis contains number axis
        ...
        if (numaxis == DEFAULT_AXIS_COUNT)
			...
			diag(qPrintable(tr("Decoded: State:%s MPos: %f,%f,%f,%f WPos: %f,%f,%f,%f\n")),
				 qPrintable(state),
				 machineCoord.x, machineCoord.y, machineCoord.z, machineCoord.a,
				 workCoord.x, workCoord.y, workCoord.z, workCoord.a
				 );
...
void GCode::gotoXYZA(QString line)
{
    pollPosWaitForIdle(false);
