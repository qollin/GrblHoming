    ...
    if (result.contains("Grbl"))
    {
        ...
        if (rx.indexIn(result) != -1 && rx.captureCount() > 0)
        {
            ...
            if (list.size() >= 3)
            {
                ...
                // diag
                err(qPrintable(tr("Got Grbl Version (Parsed:) %d.%d%c ($$=%d)\n")),
...
void GCode::parseCoordinates(const QString& received, bool aggressive)
{
    ...
    /// **************** LETARTARE   April 27, 2014  ***************************
    /// TODO
    /// verify that the fourth axis Grbl, is identical to the axis GrblController!
    /// <---
		numaxis = naxis;
        ...
        if (numaxis == DEFAULT_AXIS_COUNT)
			/// LETARTARE : diag  no work on VISTA ??
			err(qPrintable(tr("\r\nDecoded: State:%s MPos: %f,%f,%f WPos: %f,%f,%f\n")),
				 qPrintable(state),
			...
			///LETARTARE : diag   no work on VISTA ??
			err(qPrintable(tr("\r\nDecoded: State:%s MPos: %f,%f,%f,%f WPos: %f,%f,%f,%f\n")),
				 qPrintable(state),
