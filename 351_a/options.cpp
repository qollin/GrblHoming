    ...
    ui(new Ui::Options)
{
	...
	QString invA = settings.value(SETTINGS_INVERSE_A, "false").value<QString>();
	ui->chkInvA->setChecked(invA == "true");
    ...
    {
        ui->chkInvA->hide();
        ui->chkInvA->setAttribute(Qt::WA_DontShowOnScreen, true);
    }
    else
    {
        ui->chkInvA->show();
        ui->chkInvA->setAttribute(Qt::WA_DontShowOnScreen, false);
    }
...
void Options::accept()
{
	...
	settings.setValue(SETTINGS_INVERSE_A, ui->chkInvA->isChecked());
