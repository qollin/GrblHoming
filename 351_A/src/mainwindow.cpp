    ...
    queuedCommandsStarved(false), lastQueueCount(0), queuedCommandState(QCS_OK)
{
	...
	ui->lcdWorkNumberA->setDigitCount(8);
    ui->lcdMachNumberA->setDigitCount(8);
    ...
    if (!controlParams.useFourAxis)
    {
        ...
        ui->IncABtn->hide();
        ui->lblAJog->hide();
        ui->lcdWorkNumberA->hide();
        ui->lcdWorkNumberA->setAttribute(Qt::WA_DontShowOnScreen, true);
        ui->lcdMachNumberA->hide();
        ui->lcdMachNumberA->setAttribute(Qt::WA_DontShowOnScreen, true);
        ui->lblA->hide();
        ui->lblA->setAttribute(Qt::WA_DontShowOnScreen, true);
    }
	...
	connect(ui->DecABtn,SIGNAL(clicked()),this,SLOT(decA()));
    connect(ui->IncABtn,SIGNAL(clicked()),this,SLOT(incA()));
    connect(ui->btnSetHome,SIGNAL(clicked()),this,SLOT(setHome()));
    connect(ui->comboCommand->lineEdit(),SIGNAL(editingFinished()),this,SLOT(gotoXYZA()));
    ...
    connect(this, SIGNAL(gotoXYZA(QString)), &gcode, SLOT(gotoXYZA(QString)));
    ...
    if (!controlParams.useFourAxis)
    {
        ui->lcdWorkNumberA->setEnabled(false);;
        ui->lcdMachNumberA->setEnabled(false);;
        ui->IncABtn->setEnabled(false);
        ui->DecABtn->setEnabled(false);
        ui->lblAJog->setEnabled(false);
    }
...
void MainWindow::stopSending()
{
    ...
    ui->lcdWorkNumberA->setEnabled(controlParams.useFourAxis);
    ui->lcdMachNumberA->setEnabled(controlParams.useFourAxis);
    ui->IncABtn->setEnabled(controlParams.useFourAxis);
    ui->DecABtn->setEnabled(controlParams.useFourAxis);
    ui->lblAJog->setEnabled(controlParams.useFourAxis);
...
void MainWindow::decA()
{
    float coord = -ui->comboStep->currentText().toFloat();
    disableAllButtons();
    emit axisAdj('A', coord, invA, absoluteAfterAxisAdj, 0);
}
void MainWindow::incA()
{
    float coord = ui->comboStep->currentText().toFloat();
    disableAllButtons();
    emit axisAdj('A', coord, invA, absoluteAfterAxisAdj, 0);
...
void MainWindow::updateSettingsFromOptionDlg(QSettings& settings)
{
	...
	QString sinvA = settings.value(SETTINGS_INVERSE_A, "false").value<QString>();
    ...
    else
    {
        ...
        ui->IncABtn->show();
        ui->lblAJog->show();
        ui->lcdWorkNumberA->show();
        ui->lcdWorkNumberA->setAttribute(Qt::WA_DontShowOnScreen, false);
        ui->lcdMachNumberA->show();
        ui->lcdMachNumberA->setAttribute(Qt::WA_DontShowOnScreen, false);
        ui->lblA->show();
        ui->lblA->setAttribute(Qt::WA_DontShowOnScreen, false);
    }
...
void MainWindow::refreshLcd()
{
    ...
    if (controlParams.useFourAxis) {
		lcdDisplay('A', true, workCoordinates.a);
		lcdDisplay('A', false, machineCoordinates.a);
	}
	else {
		lcdDisplay('A', true, 0);
		lcdDisplay('A', false, 0);
	}
}
...
void MainWindow::lcdDisplay(char axis, bool workCoord, float floatVal)
{
    ...
    case 'A':
        if (workCoord)
            ui->lcdWorkNumberA->display(value);
        else
            ui->lcdMachNumberA->display(value);
        break;
    }
}
